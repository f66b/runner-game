...
import { GameSimulation, TICK_RATE } from './game/simulation.js';
// ============ Config ============
...
app.post('/wallet/set-balance', authMiddleware, async (req, res) => {
...
// ============ Run Routes ============
app.post('/run/start', authMiddleware, async (req, res) => {
...
app.post('/run/checkpoint/exit', authMiddleware, async (req, res) => {
...
app.post('/run/checkpoint/pause', authMiddleware, async (req, res) => {
    const { runId } = req.body;
    const session = activeSessions.get(runId);
    if (!session || session.userId !== req.userId) {
        res.status(404).json({ error: 'Session not found' });
...
app.post('/run/resume', authMiddleware, async (req, res) => {
    const { runId } = req.body;
...
wss.on('connection', async (ws, req) => {
    ...
    if (run.snapshot && run.exitType === null) {
        ...
        // New run
        simulation = new GameSimulation(combinedSeed, params, run.lockedBankroll, run.user.runCount);
    }
    const session = {
        runId: run.id,
        userId,
...
async function finalizeRun(session, exitType) {
